...
- **Fix 2 post-lanzamiento (2026-07-10, rama `fix/cuenta-abonos-inline`, feedback de Fran):** las acciones de borrar estaban escondidas en el modal "Editar". Ahora el detalle expandido del mes tiene acciones inline con confirmación en dos pasos: ✕ por abono (delete + recompute, mismo pipeline que la ✕ del modal) y "quitar pago directo" (limpia los campos de pago de la fila `payments` + recompute) — permite resetear un mes histórico y re-registrarlo con el formato nuevo SIN SQL. Los abonos no se editan: se borran y re-registran (la bitácora conserva ambos movimientos, hint en la UI).
