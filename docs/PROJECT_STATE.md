...
- **v2 en el mismo PR (feedback de Fran 2026-07-03 con screenshot + kit UI System v1):** el timeline se reescribió a la spec «tl» del kit — grid FLUIDO (columnas = 100% del ancho / días, sin px fijos ni scroll horizontal), tokens de instrumento `--baw-instr-*` (STR teal / MTR ámbar / LTR azul / hold gris / temporada verde), barras suaves con borde izquierdo 3px, half-day check-in/out (salida+entrada conviven el mismo día), leyenda-filtro interactiva, "Nn libres" por fila. CSS de componente en `globals.css` (clases `tl-*`/`cal-*`). **Price management incluido:** en Vista B se arrastra sobre días libres → panel con estimado del rango, crear/editar/eliminar temporadas (`str_seasons`), editar `base_rate_mxn`, y "Crear reservación" que prellena `/reservations` vía query params. **Fase 3 futura:** `unit_rate_overrides` por unidad (requiere migración), drag & drop de estancias, bloqueos de mantenimiento con fechas.
